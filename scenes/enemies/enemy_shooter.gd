...
@export var shooting_cooldown: float = 3.0   # Time between missile shots
...
func shoot_missile() -> void:
	if missile_scene and player_ref:
		...
		if missile == null:
			print("ERROR: missile instantiate failed")
			can_shoot = true
			return
		print("Missile instantiated, monitoring=", missile.monitoring)
		get_parent().add_child(missile);
		print("Missile added to scene tree")
		missile.global_position = global_position;
		print("Missile spawned at: ", missile.global_position, " monitoring=", missile.monitoring)
		missile.start(global_transform, player_ref);
		await get_tree().create_timer(shooting_cooldown).timeout;
		can_shoot = true;
		print("Cooldown finished, can_shoot = ", can_shoot);
	else:
